...
export function AdminDashboard() {
  ...
  const handeFacultySubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('schoolToken');
      const res = await axios.post('http://localhost:5000/api/admin/faculty', facultyForm, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleStudentSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('schoolToken');
      const res = await axios.post('http://localhost:5000/api/admin/student', studentForm, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleFoodSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('schoolToken');
      await axios.post('http://localhost:5000/api/admin/food', foodForm, {
        headers: { Authorization: `Bearer ${token}` }
      });
